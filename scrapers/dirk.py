...
async def run(*, limit: int | None, max_groups: int | None) -> None:
    ...
    async with httpx.AsyncClient(http2=False) as client:
        ...
        async def hydrate(pid: str) -> tuple[str, dict | None]:
            try:
                return pid, await fetch_detail(client, detail_sem, int(pid))
        ...
        with JsonlWriter(out_path) as writer:
            for start in range(0, len(ids), 500):
                ...
                hydrated = await asyncio.gather(*(hydrate(pid) for pid in chunk))
                for pid, detail in hydrated:
                    writer.write(envelope(rows_by_id[pid], detail))
