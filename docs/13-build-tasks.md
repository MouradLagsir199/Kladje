...
| M2 | Migration 003, **minimal**: `imports` + `import_events`. No `source_cache`, no quota index yet. Adds the deferred `recipes.import_id` FK. **Also needs `recipes.field_provenance` (JSONB)** — synthesis returns per-field provenance and there is currently nowhere to store it, which blocks Receptdetail's metadata dots and review's amber card | Upgrade + downgrade clean | `uv run alembic upgrade head` |
...
| M10 | **[B]** Validation: enum enforcement, clamps, sensible rounding, dedupe, min-viability. **Known gap to fix here:** a real TikTok yielded `servings: 18` (it was counting tacos, not people) and an ingredient with a unit but no amount. Both passed the schema | Rules in §2.5 of `docs/10-phase2-workplan.md` covered | `pytest tests/test_validation.py` |
| M11 | Import service: normalise → route by platform → fetch → synthesise → validate → draft. Foreground, in-process, **no cache**. `services/materialise.py` already does draft → rows | Draft lands in `imports.draft`, nothing in `recipes` yet | `pytest tests/test_import_service.py` |
| M12 | `POST /v1/imports`, `GET /v1/imports/{id}`, `PATCH .../draft`, `POST .../save` | Save produces a complete recipe owned by the caller | `pytest tests/test_import_save.py` |
...
| M17 | Import flow: paste + clipboard detection, progress screen, review screen (variant A). Currently a shell with honest placeholder copy | Every review edit PATCHes debounced | 📱 |
