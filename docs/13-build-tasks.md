...
| M9 | Synthesis prompt v1 from `docs/11-prompts.md` verbatim, including few-shot. **The AI may fill gaps, but must mark what it filled**: `estimated` when it had no source support, `derived` when it converted something explicit, `missing` + null when it genuinely does not know. Inventing a value and calling it `explicit` is the one unforgivable bug | Prompt loaded from a versioned module, not inline | `pytest tests/test_prompt_version.py` |
